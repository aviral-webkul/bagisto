...
export class RmaStorefrontPage extends BasePage {
    ...
    /**
     * Register a customer, place an order for the RMA enabled product and invoice
     * it, leaving an order that a return can be raised against.
     */
    ...
    /**
     * Open the new request modal for the first order still eligible for a return.
     */
    ...
    /**
     * The request detail page must render. It answered 500 for every request
     * because the item template opened a raw PHP block it never closed.
     */
    ...
    /**
     * A custom field marked required must carry a rule vee-validate knows. The
     * boolean cast on the model turned the rule name back into 1, which threw
     * "No such validator '1' exists" and left the submit button doing nothing.
     */
    ...
    /**
     * A multiselect custom field has to post under customAttributes like every
     * other type, or the answer the customer picked is dropped on submit.
     */
